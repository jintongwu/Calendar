...
public class User implements IUser {
    ...
    public List<Calendar[]> getAllFreeTime(Calendar startTime, Calendar endTime) {
        // if the start time is after the end time or list of users is empty, return
        // null
        ...
        while (currIndex < events.size() && events.get(currIndex).getEndTime().before(endTime)
                && events.get(currIndex).getEndTime().after(startTime)) {
